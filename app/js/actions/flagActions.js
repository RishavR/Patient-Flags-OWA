...
export function getFlags(){
    return dispatch => {
        if(dataLoaded)
            return;
        dataLoaded=true;
        var url='http://localhost:8081/openmrs/ws/rest/v1/patientflags/flag?v=full'; // TODO: pick up base URL from {Origin}
...
export function deleteFlag(uuid){
    var url='http://localhost:8081/openmrs/ws/rest/v1/patientflags/flag/'+encodeURI(uuid); // TODO: pick up base URL from - Dynamic Path ${Origin}
        var auth='Basic YWRtaW46QWRtaW4xMjM='; // TODO: pick up from user login credentials 
        console.log("Successful Entry");
        fetch(url, {
            method: 'DELETE',
...
export function updateFlag(destString,updateData){
    return dispatch =>{
        ...
        var url='';
        if(destString==='')
            url = 'http://localhost:8081/openmrs/ws/rest/v1/patientflags/flag/';
        else 
            url='http://localhost:8081/openmrs/ws/rest/v1/patientflags/flag/'+encodeURI(destString); // TODO: pick up base URL from {Origin}
        console.log(url);
        var auth='Basic YWRtaW46QWRtaW4xMjM='; // TODO: pick up from user login credentials 
        console.log("Successful Entry");
        fetch(url, {
